...
class WPLMS_tips{
	...
	function add_unsubscribe_link($user_id,$course_id){
		...
		?>
		<a href="javascript:void(0);" class="link unsubscribe_me_course" data-course="<?php echo $course_id;?>"><?php echo _x('Unsubscribe from course','','vibe-customtypes')?></a>
		<?php
	}
		...
		<script>
			jQuery(document).ready(function($){
				$('.unsubscribe_me_course').on('click',function (){
					...
					if(!course){
						alert('<?php echo _x('Course missing.Could not unsubscribe!','','vibe-customtypes');?>');
					}else{
						var un_cn_text = '<?php echo _x('Are you sure you want to unsubscribe from course','','vibe-customtypes');?>';
				        $.confirm({
				          text: un_cn_text,
				          confirm: function() {
				            jQuery.ajax({
				          ...
				          confirmButton: '<?php echo _x('Yes','','vibe-customtypes');?>',
				          cancelButton: '<?php echo _x('Cancel','','vibe-customtypes');?>'
				        });
	...
	function wplms_unsubscribe_me_course(){
		if(!is_user_logged_in()){
			$data = array('status'=>false,'message'=>_x('User not logged in','','vibe-customtypes'));
		...
		if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'wplms_unsubscribe_me_course'.$user_id) ){
			$data = array('status'=>false,'message'=>_x('Security check failed','','vibe-customtypes'));
			echo json_encode($data);
			die();
		}
		if(!empty($course_id ) && is_numeric($course_id )){
			bp_course_remove_user_from_course($user_id,$course_id);
			$data = array('status'=>true,'message'=>_x('Successfully unsubscribed from course!','','vibe-customtypes'));
		}else{
			$data = array('status'=>false,'message'=>_x('Something went wrong!','','vibe-customtypes'));
		}
		echo json_encode($data);
		die();
