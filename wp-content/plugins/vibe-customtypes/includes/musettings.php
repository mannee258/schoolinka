...
class lms_settings{
	...
	function lms_general_settings(){
		...
		switch($_GET['sub']){
			...
			default:
				...
				array(
						'label' => __('Show PDF Certificates only','vibe-customtypes'),
						'name' =>'course_pdf_certificates',
						'type' => 'checkbox',
						'desc' => __('Show only PDF certificates','vibe-customtypes')
					),
				array(
				...
				array(
						...
						'type' => 'checkbox',
						'desc' => __('Enable direct link to completed unit from curriculum','vibe-customtypes').'<a href=" https://wplms.io/support/knowledge-base/direct-unit-link-from-curriculum/ " target="_blank"><i class="dashicons  dashicons-editor-help"></i></a>(NEEDS "Show unit slug and title in Course Status page" to be enabled)'
					),
				array(
						'label' => __('Enable Direct link to all units in curriculum(if student subscribed to course)','vibe-customtypes'),
						'name' =>'completed_unit_link_course_status_for_all_units',
						'type' => 'checkbox',
						'desc' => __('Enable direct link to all units from curriculum (NEEDS "Show unit slug and title in Course Status page" to be enabled)','vibe-customtypes')
					),
				array(
				...
				array(
			            'label' => __('Revert to old commissions graphs(from orders table)', 'vibe-customtypes'),
			            'name' => 'wplms_commissions_migrate_to_activity_revert',
			            'desc' => __('It will revert to old commissions graphs in dashboard(We encourage you to migrate the commissions to activity from wp-admin -> lms -> settings -> functions -> Sync Areas )', 'vibe-customtypes'),
			            'type' => 'checkbox',
					),
				...
				array(
						'label' => __('Enable Unsubscribe link for students','vibe-customtypes'),
						'name' => 'unsubscribe_student_link',
						'type' => 'checkbox',
						'desc' => __('Students will be able to unsubscribe from courses from profile -> courses -> stats','vibe-customtypes')
					),
				array(
	...
	*/
	function lms_registration_forms(){
		...
		$registration_mails = array('no'=>__('Disable activation email and manually approve accounts.','vibe-customtypes'));
		...
		switch($_GET['sub']){
			...
			default:
				...
				$sync_settings = apply_filters('wplms_sync_settings',array(
					...
					array(
						'id'=>'unit_attachments',
						'label'=>__('Unit attachments','vibe-customtypes'),
						'description'=>__('Re-link the unit attachments in proper way.','vibe-customtypes'),
					),
					array(
						'id'=>'woocommerce_instructor_commissions',
						'label'=>__('Woocommerce Intructor commissions(for non-bundled products)','vibe-customtypes'),
						'description'=>__('Migrates woocommerce instructor commissions to activity meta from order meta for non-bundled(products having single course associated) products only.','vibe-customtypes'),
					),
					array(
						'id'=>'woocommerce_instructor_commissions_bundle',
						'label'=>__('Woocommerce Intructor commissions(for bundled products)','vibe-customtypes'),
						'description'=>__('Migrates woocommerce instructor commissions to activity meta from order meta for bundled(products having more than one course associated) products only.','vibe-customtypes'),
					),
				));
	...
	function lms_commission_payments(){
		...
		if(!$migrated_to_activity){
			...
			if($migrated_to_activity){
				...
				echo '<br><br><br><strong>'.__('SET TIME PERIOD','vibe-customtypes').' :</strong><input type="text" name="start_date" id="from" value="'.$start_date.'" class="date-picker-field">
						 <label for="to">&nbsp;&nbsp; To:</label> 
						<input type="text" name="end_date" id="to" value="'.$end_date.'" class="date-picker-field">
						<input type="submit" class="button" name="set_time" value="Show"></p><br>';
			}
			if(!empty($new_instructor_data) && is_array($new_instructor_data)){
				...
				if(!empty($instructor_c_commissions)){
					...
					<h3><label>'.__('Instructor Name','vibe-customtypes').'</label><span>'.__('Commission','vibe-customtypes').' ('.get_woocommerce_currency_symbol($current_currency).')</span><span>'.__('PAYPAL EMAIL','vibe-customtypes').'</span><span>'.__('Select','vibe-customtypes').'</span><span>'.__('Pay via PayPal','vibe-customtypes').'</span></h3>
					<div class="inside">
						<ul>';
					//loop 
					$instructors = get_users('role=instructor');		
					foreach ($instructors as $instructor) {
				        ...
				        echo '<li><label>'. $instructor->user_nicename.'</label>
                        <span><input type="text" id="'.$instructor->ID.'_amount" name="instructor['.$instructor->ID.'][commission]" class="text" value="'.(isset($instructor_c_commissions[$instructor->ID])?$instructor_c_commissions[$instructor->ID]:0).'" /></span>
	...
	function live_addons(){
		$addons = apply_filters('wplms_live_addons',array(
				...
				'wplms_phone_auth' =>array(
					'label'=> __('Wplms Phone Auth','vibe-customtypes'),
					'sub'=> __('Integrates Firebase phone verification system with wplms.','vibe-customtypes'),
					'icon'=> '<span class="dashicons dashicons-portfolio"></span>',
					'requires'=> '',
	...
	function get_addons(){
		return apply_filters('wplms_lms_addons',array(
			'wplms-appointments' =>array(
					'label'=> __('WPLMS Appointments','vibe-customtypes'),
					'sub'=> __('Book Instructors in WPLMS','vibe-customtypes'),
					'icon'=> '<span class="dashicons dashicons-portfolio"></span>',
					'requires'=> '',
			...
			'wplms-pdf-certificates' =>array(
					'label'=> __('WPLMS PDF Certificates','vibe-customtypes'),
					'sub'=> __('High Quality Pdf certificates','vibe-customtypes'),
					'icon'=> '<span class="dashicons dashicons-portfolio"></span>',
					'requires'=> '',
			...
			'wplms-parent-user' =>array(
					'label'=> __('WPLMS Parent User','vibe-customtypes'),
					'sub'=> __('Parent/Guardian user in WPLMS','vibe-customtypes'),
					'icon'=> '<span class="dashicons dashicons-portfolio"></span>',
					'requires'=> '',
			...
			'wplms-attendance' =>array(
					'label'=> __('WPLMS Attendance','vibe-customtypes'),
					'sub'=> __('Adds attendance feature in courses of wplms','vibe-customtypes'),
					'icon'=> '<span class="dashicons dashicons-portfolio"></span>',
					'requires'=> '',
...
class wplms_miscellaneous_settings{
	...
	function sync_resync(){
		...
		switch($_POST['id']){
			...
			case 'woocommerce_instructor_commissions_bundle':
                ...
                INNER JOIN {$order_items_table} as item 
                ON meta.order_item_id = item.order_item_id
                WHERE meta.meta_key like '%commission%' GROUP BY item.order_id
                "); 
			...
			case 'unit_attachments':
				global $wpdb;
				$data = $wpdb->get_results("SELECT p.ID as unit_id FROM {$wpdb->posts} as p LEFT JOIN {$wpdb->postmeta} as pm ON p.ID = pm.post_id WHERE p.post_type = 'unit'");
				$unit_data = array();
				if(!empty($data)){
	...
	function end_unit_attachments_sync(){
		...
		echo __('Unit - attachments sync complete !','vibe-customtypes');
		die();
	}
	...
	function end_woocommerce_instructor_commissions_sync(){
		...
		echo __('Instructror commissions migrated  !','vibe-customtypes');
		die();
	}
