    ...
    public static function init(){
        ...
        function evaluate_assignment(){
           ...
           if ( !isset($_POST['security']) || !wp_verify_nonce($_POST['security'],'vibe_assignment') ){
            ...
            <input type="number" class="form_field" id="assignment_marks" placeholder="'.__('Enter marks out of ','wplms-assignments').$max_marks.'" value="'.$user_marks.'"/>
            <label>'.__('REMARKS (if any)','wplms-assignments').'</label>';
            if(!empty($remarkmessageid)){
                echo '<br><a href="'.trailingslashit( bp_loggedin_user_domain() . $bp->messages->slug . '/view/' . $remarkmessageid ).'" class="button small">'.__('See Old Remarks','vibe').'</a><span class="clearfix"></span>';
            }
        ...
        function get_assignment_submission_count($course_id){
            ...
            $count = $wpdb->get_var("SELECT count(p.meta_key) FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->comments} as c ON p.post_id = c.comment_post_ID 
                  WHERE CAST(c.user_id as UNSIGNED) = CAST(p.meta_key as UNSIGNED) 
                  AND c.comment_approved='1'
                  AND meta_value LIKE '0' AND meta_key REGEXP '^[0-9]+$' AND p.post_id IN ($assignment_ids)");
        ...
        function get_assignment_submissions($course_id){
            ...
            if(!empty($assignments)){
                ...
                $count_array = array();
                $assignment_ids = implode(',',$assignment_ids);
                $submissions = $wpdb->get_results("SELECT DISTINCT p.meta_key as count, p.post_id FROM {$wpdb->postmeta} as p
                ...
                <div class="submissions_form">
                    ...
                    foreach($assignments as $assignments_id){
                        ?>
                        <option value="<?php echo $assignments_id->post_id; ?>"><?php echo get_the_title($assignments_id->post_id); ?> (<?php echo (empty(count($count_array[$assignments_id->post_id]))?0:count($count_array[$assignments_id->post_id])); ?>)</option>
                        <?php   
                    }
                    ?>
                    </select>
                    <select id="fetch_assignment_status">
                        ...
                        <option value="2"><?php echo _x('Unsubmitted','assignment status','wplms-assignments') ?></option>
                    </select>
