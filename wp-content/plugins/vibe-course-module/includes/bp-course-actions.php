    ...
    */
	...
	function bp_course_get_course_quiz_submissions($course_id){
		...
		if(!empty($quizes)){
			...
			global $wpdb;
			$count_array = array();
	  		$submissions = $wpdb->get_results(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT count(*) as count,u.meta_key as quiz_id FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->usermeta} as u ON p.meta_key = u.user_id WHERE p.meta_value LIKE '0' AND u.meta_key = p.post_id AND u.meta_value < %d AND p.post_id IN ($quiz_ids) GROUP BY u.meta_key",time())));
