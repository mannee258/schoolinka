...
// After begin quiz, get unmarked quesitons
    ...
    function fetch_quiz_submissions(){
      ...
      switch($status){
        case 0:
          $members = $wpdb->get_results( apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT DISTINCT u.user_id as meta_key FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->usermeta} as u ON p.meta_key = u.user_id WHERE p.meta_value LIKE '0' AND u.meta_key = p.post_id AND u.meta_value < %d AND p.post_id = %d",time(),$quiz_id)), ARRAY_A); // Internal Query
        break;
        case 1:
