...
function bp_course_get_quiz_submission_count($course_id){
  ...
  if(!empty($quizes)){
    ...
    $count = $wpdb->get_var(apply_filters('wplms_usermeta_direct_query',$wpdb->prepare("SELECT count(*) FROM {$wpdb->postmeta} as p LEFT JOIN {$wpdb->usermeta} as u ON p.meta_key = u.user_id WHERE p.meta_value LIKE '0' AND u.meta_key = p.post_id AND u.meta_value < %d AND p.post_id IN ($quiz_ids)",time())));
  }
  return (empty($count)?0:$count);
}
...
function wplms_get_course_schema_details($course_id){
    ...
    if(!empty($course_author) && function_exists('bp_core_get_user_displayname')){
        ...
        if(!empty($course_author_name)){
            ...
            $field_value = (bp_is_active('xprofile')?bp_get_profile_field_data('field='.$field.'&user_id='.$instructor_id):'');
