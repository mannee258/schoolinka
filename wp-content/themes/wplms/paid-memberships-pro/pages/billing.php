<?php 				
	...
	{ 
	?>
		<div class="message"><p><?php printf(__("Logged in as <strong>%s</strong>.", "vibe"), $current_user->user_login);?> <small><a href="<?php echo wp_logout_url(home_url(). "/membership-checkout/?level=" . $level->id);?>"><?php _e("logout", "vibe");?></a></small></p></div>
		<hr />
		<ul class="pmpro_account_list">
			<li><strong><?php _e("Level", "vibe");?>:</strong> <?php echo vibe_sanitizer($level->name)?></li>
			...
			<li><strong><?php _e("Membership Fee", "vibe");?>:</strong>
				<?php
					...
					if($current_user->membership_level->cycle_number > 1) {
						...
						echo vibe_sanitizer($pmpro_currency_symbol),vibe_sanitizer( $current_user->membership_level->billing_amount);
					}
				?>
			</li>
		<?php } ?>						
		...
		<?php if($level->billing_limit) { ?>
			<li><strong><?php _e("Duration", "vibe");?>:</strong> <?php echo vibe_sanitizer($level->billing_limit.' '.sornot($level->cycle_period,$level->billing_limit))?></li>
		<?php } ?>		
		</ul>
	<?php 
	} 
